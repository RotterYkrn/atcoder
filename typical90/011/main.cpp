...
#define inputl(...) long __VA_ARGS__; input_cin(__VA_ARGS__);
...
int main() {
    inputi(N);
    auto work = inputv<long long>({N,3});
    sort(all(work));

    auto dp = mkvec<long long>({5009, 5009});
    rep(w,N) {
        rep(day, 5001) {
            chmax(dp[w + 1][day], dp[w][day]);
            if (day + work[w][1] <= work[w][0]) {
                chmax(dp[w + 1][day + work[w][1]], dp[w][day] + work[w][2]);
            }
        }
    }
