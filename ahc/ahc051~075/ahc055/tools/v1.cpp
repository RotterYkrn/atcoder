...
int main() {
    inputi(N);
    auto box_health = inputv<int>(N);
    auto weapon_health = inputv<int>(N);
    auto damages = inputv<int>({N, N});
    ...
    while (got_num < N) {
        ...
        while (!que.empty()) {
            ...
            if (attack_box == -1) {
                attack_box = max_damage_only_closed_idx(damages[w], box_health);
            }

            int attack_num = min((box_health[attack_box] - 1) / damages[w][attack_box] + 1, weapon_health[w]);
            print_attack(w, attack_box, attack_num);
            box_health[attack_box] -= attack_num * damages[w][attack_box];
            weapon_health[w] -= attack_num;

            if (box_health[attack_box] <= 0) {
                if (weapon_health[w] > 0) {
                    que.push(w);
                }
                break;
            }
        ...
        if (attack_box != -1) {
            print_attack(-1, attack_box, box_health[attack_box]);
        } else {
            attack_box = min_only_pos_idx(box_health);
            print_attack(-1, attack_box, box_health[attack_box]);
        }
        box_health[attack_box] = 0;
