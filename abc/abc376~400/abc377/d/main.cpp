...
int main() {
    inputi(N,M);
    auto comb = mkvec<pii>(0);
    auto imos = mkvec<int>({N + 1, 2});
    rep(M) {
        inputi(L,R);
        imos[L - 1][0] += 1;
        imos[R - 1][1] += 1;
    ...
    int left = 0, tmp_left = 0, imos_sum = 0;
    bool mnt_l = false, mnt_r = false;
    rep(i, N + 1) {
        imos_sum += imos[i][0] - imos[i][1];
        if (imos_sum != 0 && !mnt_l && !mnt_r) {
            mnt_l = mnt_r = true;
        }
        if (mnt_r && imos[i][1] != 0) {
            comb.pb({left, i});
