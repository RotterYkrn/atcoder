...
int main() {
    ...
    auto s1 = mkvec<ll>(N + 1), s2 = mkvec<ll>(N + 1);
    auto ss = mkvec<int>(0);
    rep(i,N) {
        if (S[i] == '1') s1[i + 1] = 1;
        else if (S[i] == '2') s2[i + 1] = 1;
        else ss.pb(i);
        s1[i + 1] += s1[i];
        s2[i + 1] += s2[i];
    }
    ...
    rep(Q) {
        inputi(L, R); L--;
        auto sl = lower_bound(all(ss), L);
        auto sr = lower_bound(all(ss), R);
        ll ans = 0;
        for (auto it = sl; it != sr; ++it) {
            ll l1 = s1[*it] - s1[L];
            ll r2 = s2[R] - s2[*it + 1];
            chmax(ans, min(l1, r2) * 2 + 1);
        }
        print(ans);
    }
