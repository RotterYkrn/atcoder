...
int main() {
    ...
    auto before = mkvec<pair<int, int>>({N, 0});
    auto dp = mkvec<ll>({N, 0});
    int sz = 1;
    rep(i, N) {
        before[i] = mkvec<pair<int, int>>(sz);
        dp[i] = mkvec<ll>(sz, -1LL);
        sz *= 3;
    }

    rep(i, before[N - 1].size()) {
        int cnt = 0;
        rep(j, 3) cnt += (S[i * 3 + j] == '1');
        before[N - 1][i] = {(cnt >= 2), cnt};
    }
    rrep(n, N - 1, 0) {
        rep(i, before[n].size()) {
            int cnt = 0;
            ...
            before[n][i] = {(cnt >= 2), bit};
        }
    }

    auto f = [&](auto self, int n, int i, int bbit) -> ll {
        bitset<3> bbs(bbit);
        if (before[0][0].first == 0) bbs.flip();
        if (n == N) return bbs.count();
        if (dp[n][i] != -1) return dp[n][i];

        auto ch = mkvec<ll>(3, infl);
        auto [st, bit] = before[n][i];
        rep(j, 3) {
            if (st != before[0][0].first || (bit & (1 << j)) ^ before[0][0].first) continue;
            ch[j] = self(self, n + 1, i * 3 + j, bit);
        }

        bitset<3> bs(bit);
        if (before[0][0].first == 0) bs.flip();

        if (bs.count() == 2) {
            return min({ch[0], ch[1], ch[2]});
        } else {
            return min({ch[0] + ch[1], ch[0] + ch[2], ch[1] + ch[2]});
        }
    };

    print(f(f, 0, 0, before[0][0].second));
