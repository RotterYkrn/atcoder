...
int main() {
    ...
    // 1->none, 2->0, 3~->1
    auto f = [&](auto self, int bit, int n_lim, int k_lim) -> pair<vl, int> {
        auto vbit = mkvec<int>(0);
        rep(i, N) {
            if (bit & (1 << i)) {
                vbit.pb(i);
            }
        ...
        rrep(i, n_lim + 1, 0) {
            auto kbit = mkvec<int>(n_lim);
            fill(rtoit(kbit, n_lim - i - 1), 1);
            do {
                int nbit = bit;
                rep(j, n_lim) {
                    if (kbit[j] == 1) {
            ...
            } while (next_permutation(all(kbit)));
        }
    };
