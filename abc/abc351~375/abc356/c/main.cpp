...
int main() {
    inputi(N, M, K);
    auto tests = mkvec<pair<vi, int>>(M);
    rep(i, M) {
        inputi(c);
        auto tmp = inputv<int>(c);
        inputs(s);
        tests[i] = {tmp, s == "o" ? 1 : 0};
    }

    auto bit = mkvec<int>(N);
    int ans = 0;
    rep(k, 0, N + 1) {
        auto tmp = bit;
        do {
            auto correct_keys = mkvec<int>(0);
            rep(i, (int)tmp.size()) {
                if (tmp[i]) correct_keys.push_back(i + 1);
            }
            bool ok = true;
            for (auto [A, R] : tests) {
                int cnt = 0;
                for (auto a : A) {
                    if (find(all(correct_keys), a) != correct_keys.end()) cnt++;
                }
                if ((R && cnt < K)
                    || (!R && cnt >= K)) {
                    ok = false;
