...
int main() {
    ...
    rep(T) {
        ...
        auto front = mkvec<ll>({2, N + 1});
        auto back = mkvec<ll>({2, N + 1});

        ll f0 = 0, f1 = 0, b0 = 0, b1 = 0;
        rep(i, 1, N + 1) {
            if (S[i - 1] == '0') {
                front[0][i] = front[0][i - 1];
                front[1][i] = front[1][i - 1] + f1 * 2 + 1LL;
                f0++;
                f1 = 0;
            } else {
                front[0][i] = front[0][i - 1] + f0 * 2 + 1LL;
            ...
            if (S[j] == '0') {
                back[0][j] = back[0][j + 1];
                back[1][j] = back[1][j + 1] + b1 * 2 + 1LL;
                b0++;
                b1 = 0;
            } else {
                back[0][j] = back[0][j + 1] + b0 * 2 + 1LL;
                back[1][j] = back[1][j + 1];
                b1++;
                b0 = 0;
            }
        ...
        ll ans = infl;
        rep(i, N + 1) {
            chmin(ans, front[0][i] + back[0][i]);
            chmin(ans, front[1][i] + back[1][i]);
        }
