...
auto rec(const int i, const int N, const int M, const auto &h) {
    ...
    int low_lim = 1;
    if (i != 0) low_lim = h[i - 1] + 10;

    rep(a, low_lim, M - 10 * (N - i - 1) + 1) {
        auto prev_h = h;
        prev_h.pb(a);
        auto tmp = rec(i + 1, N, M, prev_h);
...
int main() {
    ...
    auto ans = rec(0, N, M, mkvec<int>(0));
    print(ans.size());
    print(ans);
