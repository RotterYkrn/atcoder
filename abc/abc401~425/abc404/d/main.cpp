...
int main() {
    inputi(N, M);
    auto prices = inputv<ll>(N);
    auto zoo_parks = mkvec<int>({N,0});
    rep(i, M) {
        inputi(k);
        rep(k) {
            inputi(a);
            --a;
            zoo_parks[a].push_back(i);
        }
    }

    auto rec = [&](auto self, const int i, const ll cost, const auto seen) -> ll {
        if (all_of(all(seen), [](auto &i) { return i >= 2; })) {
            return cost;
        } else if (i == N) {
            return infl;
        }
        ...
        rep(visit_num, 3LL) {
            auto s = seen;
            for (auto z : zoo_parks[i]) {
                if (z < M) s[z] += visit_num;
            }
            chmin(res, self(self, i + 1, cost + prices[i] * visit_num, s));
        }

        return res;
    };

    auto seen = mkvec<int>(M);
    auto ans = rec(rec, 0, 0, seen);
    print(ans);
