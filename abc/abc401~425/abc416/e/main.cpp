...
int main() {
    ...
    for (int k = 0; k < N + 1; k++) {
        for (int i = 0; i < N + 1; i++) {
            for (int j = 0; j < N + 1; j++) dp[i][j] = min(dp[i][j], dp[i][k] + dp[k][j]);
        }
    }
    ...
    rep(Q) {
        ...
        if (q == 1) {
            ...
            chmin(dp[b - 1][a - 1], c);
            rep(i, N + 1) rep(j, N + 1) {
                chmin(dp[i][j], dp[i][a - 1] + dp[a - 1][b - 1] + dp[b - 1][j]);
                chmin(dp[i][j], dp[i][b - 1] + dp[b - 1][a - 1] + dp[a - 1][j]);
                dp[j][i] = dp[i][j];
            }
        } else if (q == 2) {
            inputll(x);
            ...
            dp[N][x] = 0;
            rep(i, N + 1) rep(j, N + 1) {
                chmin(dp[i][j], dp[i][x] + dp[x][N] + dp[N][j]);
                chmin(dp[i][j], dp[i][N] + dp[N][x] + dp[x][j]);
                dp[j][i] = dp[i][j];
            }
        } else {
            ll ans = 0;
