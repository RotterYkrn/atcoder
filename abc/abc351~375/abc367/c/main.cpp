...
auto rec(const int i, const int sum, const int N, const int K, const auto &A, const auto pic) {
    auto res = mkvec<int>({0, N});

    if (i == N) {
        if (sum % K == 0) res.pb(pic);
        return res;
    }
    ...
    rep(n,1,A[i]+1) {
        auto p = pic;
        p.pb(n);
        auto tmp = rec(i + 1, sum + n, N, K, A, p);
