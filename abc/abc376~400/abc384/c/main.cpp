...
int main() {
    auto score = inputv<int>(5);

    auto names = vector<string>{"ABCDE",
    "ABCD", "ABCE", "ABDE", "ACDE", "BCDE",
    "ABC", "ABD", "ABE", "ACD", "ACE", "ADE", "BCD", "BCE", "BDE", "CDE",
    "AB", "AC", "AD", "AE", "BC", "BD", "BE", "CD", "CE", "DE",
    "A", "B", "C", "D", "E"};

    auto score_name = mkvec<pair<int, string>>(names.size());
    rep(i, names.size()) {
        int sum = 0;
        for (auto c : names[i]) {
            sum += score[c - 'A'];
        }
        score_name[i] = {sum, names[i]};
    }

    sort(all(score_name), [](auto a, auto b) {
        if (a.first != b.first) return a.first > b.first;
        else return a.second < b.second;
    });
    for (auto [s, n] : score_name) {
        print(n);
    }
