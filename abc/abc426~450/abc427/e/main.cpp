    ...
    rep(i, H) {
        rep(j, W) {
            if (grid[i][j] == 'T') {
                ...
                trashes.push_back({i, j, 0});
            }
        }
    }

    auto erase_trash = mkvec<unordered_set<int>>({H * 3,W * 3});
    rep(ti, trashes.size()) {
        auto [i, j, _] = trashes[ti];
        int t, d, l, r;
        ...
        for (int k = t + H; k < d + H; k++) {
            erase_trash[k][l].insert(ti);
            erase_trash[k][r].insert(ti);
        }
        for (int k = l + W; k < r + W; k++) {
            erase_trash[t][k].insert(ti);
            erase_trash[d][k].insert(ti);
        }
    }

    vector<bool> erased(trashes.size(), false);
    auto seen = mkvec<bool>({H * 3, W * 3}, false);
    queue<State> que;
    que.push({T.i + H, T.j + W, 0});
    int t = 0, d = H - 1, l = 0, r = W - 1, ans = -1;
    while (!que.empty()) {
        auto [i, j, dist] = que.front();
        que.pop();

        for (auto &t : erase_trash[i][j]) {
            erased[t] = true;
        }

        if (all_of(all(erased), [](bool b) { return b; })) {
            ans = dist;
            break;
        }
        ...
        for (int k = 0; k < 4; k++) {
            ...
            if (!ir(ni, H * 3) || !ir(nj, W * 3)) continue;
            if (ir(ni - H, t, d + 1) || ir(nj - W, l, r + 1)) {
                if (grid[ni - H][nj - W] == '#') continue;
            }
            seen[ni][nj] = true;
