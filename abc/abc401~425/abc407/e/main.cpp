...
int main() {
    ...
    auto rec = [&](auto self, int i, int op, int cl, ll score) -> ll {
        if (i == N * 2) {
            return score;
        }
        ...
        if (op > 0) {
            chmax(res, self(self, i + 1, op - 1, cl + 1, score + A[i]));
        }
        if (cl > 0) {
            chmax(res, self(self, i + 1, op, cl - 1, score));
        }
