...
int main() {
    auto p = inputv<int>({3, 2});
    auto v = mkvec<int>({3,2});
    rep(i, 3) {
        v[i][0] = p[i][0] - p[(i + 1) % 3][0];
        v[i][1] = p[i][1] - p[(i + 1) % 3][1];
    }

    YesNo(inner_product(all(v[0]), v[1].begin(), 0) == 0
        || inner_product(all(v[0]), v[2].begin(), 0) == 0
        || inner_product(all(v[1]), v[2].begin(), 0) == 0)
