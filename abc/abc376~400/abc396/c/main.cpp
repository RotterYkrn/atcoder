...
struct CumulativeSumArray1 {
    ...
    T query(const auto l, const auto r) const {
        return arr[r] - arr[l];
    }
};
...
int main() {
    inputi(N, M);
    auto Black = inputv<ll>(N);
    auto White = inputv<ll>(M);

    sort(all(Black));
    sort(all(White));

    CumulativeSumArray1<ll> csB(Black);
    CumulativeSumArray1<ll> csW(White);

    auto idxB = (int)distance(Black.begin(), lower_bound(Black.begin(), Black.end(), 0LL));
    auto idxW = (int)distance(White.begin(), lower_bound(White.begin(), White.end(), 0LL));

    if (N - idxB >= M - idxW) {
        print(csB.query(idxB, N) + csW.query(idxW, M));
    } else {
        ll ans = -infl;
        rep(i, idxB - idxW + 2) {
            if (idxB - i < 0 || M - (N - idxB) - i < 0) break;
            chmax(ans, csB.query(idxB - i, N) + csW.query(M - (N - idxB) - i, M));
        }
        print(ans);
    }
