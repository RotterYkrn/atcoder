...
void printTrent(const vector<Point> &v) {
    cout << v.size();
    for (const auto &i : v) {
...
void actDfs(const Point p, const Point g, const vector<string> &B, auto &seen) {
    ...
    rep(d, 4) {
        ...
        actDfs({nx, ny}, g, B, seen);

        if (seen[g.first][g.second]) return;
    }
}

bool dfs(const Point cur, const Point g, const vector<string> &B) {
    auto seen = mkvec<bool>({B.size(), B.size()}, false);
    actDfs(cur, g, B, seen);
...
int main() {
    ...
    for (const auto &i : initTrent) {
        auto tmp = B;
        auto appended = mkvec<Point>(0);
        for (const auto &j : i) {
            if (!ir(j.first, N)
              || !ir(j.second, N)
              || tmp[j.first][j.second] == 'T'
              || j == t) continue;
            tmp[j.first][j.second] = 'T';
            appended.pb(j);
        }
