...
ll ans = 0;
void rec(const int i, const int j, const int d, const int K, const auto &S, auto seen) {
    if (!ir(i,(int)S.size()) || !ir(j,(int)S[i].size()) || S[i][j] =='#' || seen[i][j]) return;
    if (d == K) {
        ans++;
        return;
    }
    seen[i][j] = 1;
    rep(k,4) {
        rec(i + dx[k], j + dy[k], d + 1, K, S, seen);
    }
    seen[i][j] = 0;
}
