...
bool isBlackSquare(const vector<string> &S, int i, int j) {
    return S[i][j] == '#' &&
        S[i][j + 1] == '#' &&
        S[i + 1][j] == '#' &&
        S[i + 1][j + 1] == '#';
...
int main() {
    inputi(T);
    auto f = [](auto f, size_t i, vector<string> &S, const vector<pii> &s, int ans) -> int {
        if (i == s.size()) {
            return ans;
        }
