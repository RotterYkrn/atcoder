...
struct UnionFind {
    ...
    UnionFind(const int N) : par(N), large(N), group(N) { //最初は全てが根であるとして初期化
        for(int i = 0; i < N; i++) {
            par[i] = i;
            large[i].pb(i + 1);
        }
    ...
    void unite(const int x, const int y) { // xとyの木を併合
        ...
        auto large_xy = large[rx];
        large[rx].insert(large[rx].end(), all(large[ry]));
        sort(rall(large[rx]));
        if (large[rx].size() > 10) {
            large[rx].erase(large[rx].begin() + 10, large[rx].end());
        }
...
int main() {
    ...
    rep(Q) {
        ...
        if (q == 1) {
            ...
            inputi(v,k); v--;
            if ((int)uni.large[uni.root(v)].size() < k) print(-1);
            else print(uni.large[uni.root(v)][k - 1]);
        }
    }
