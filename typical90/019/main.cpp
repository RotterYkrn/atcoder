...
ll rec(const int l, const int r, const auto &A, auto &dp) {
    if (dp[l][r] != -1LL) return dp[l][r];
    if (r - l == 2) return dp[l][r] = abs(A[l] - A[r - 1]);

    ll res = abs(A[l] - A[r - 1]) + rec(l + 1, r - 1, A, dp);
    for (int i = l + 2; i < r; i += 2) {
        chmin(res, rec(l, i, A, dp) + rec(i, r, A, dp));
    }
...
int main() {
    inputi(N);
    auto A = inputv<int>(2 * N);
    auto dp = mkvec<ll>({2*N + 10, 2*N + 10}, -1LL);
    print(rec(0, 2 * N, A, dp));
