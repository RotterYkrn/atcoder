...
struct Combination {
    ...
    explicit Combination(const auto N) : fac(N), finv(N) {
    ...
    inline mint calc(const auto n, const auto k) const noexcept {
        if (n < k) return 0;
        if (n < 0 || k < 0) return 0;
...
int main() {
    ...
    Combination com(60);
    mint ans(0);
    bitset<60> hisN(0), hisM;
    int i = 0;
    while (N > 0) {
        if (N & 1 && M & 1) ans += hisN.to_ullong() + 1;
        mint tmp(0);
        if (N & 1) {
            rep(i, 1, hisM.count() + 1) tmp += com.calc(hisM.count(), i) * i;
        }
        ans += tmp * mint(2).pow(i - hisM.count());
        hisN.set(i, N & 1);
        hisM.set(i, M & 1);
        N >>= 1;
        M >>= 1;
