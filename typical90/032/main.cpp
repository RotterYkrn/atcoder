...
int rec(const int bit, const int sec, const int r, const int rt, const int N, const auto st, const auto h) {
    if (sec == N - 1) return rt + st[r][sec];

    int nbit = bit & ~(1 << r);
    int res = inf;
    rep(nr, N) {
        if ((nbit & (1 << nr)) && !h[r][nr]) {
            chmin(res, rec(nbit, sec + 1, nr, rt + st[r][sec], N, st, h));
        }
    }
...
int main() {
    inputi(N);
    auto st = inputv<int>({N,N});
    inputi(M);
    auto hate = mkvec<int>({N,N});
    rep(M) {
        inputi(X,Y);
        hate[X - 1][Y - 1] = hate[Y - 1][X - 1] = 1;
    }

    int ans = inf;
    rep(r, N) {
        chmin(ans, rec((1 << N) - 1, 0, r, 0, N, st, hate));
    }
