...
int main() {
    ...
    auto dist = mkvec<int>({H, W}, -1);
    auto do_push = [&](int i, int j) {
        return ir(i, H) && ir(j, W) && dist[i][j] == -1;
    };

    priority_queue<State, vector<State>, decltype(compare)> que{compare};
    que.push({si, sj, 0});
    while (!que.empty()) {
        auto [i, j, d] = que.top(); que.pop();
        ...
        rep(k, 4) {
            ...
            if (!do_push(ni, nj)) continue;
            if (S[ni][nj] == '#') {
                que.push({ni, nj, d + 1});
                int nni = ni + dy[k], nnj = nj + dx[k];
                if (do_push(nni, nnj)) {
                    que.push({nni, nnj, d + 1});
                }
            } else {
                que.push({ni, nj, d});
            }
