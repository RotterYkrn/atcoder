...
int main() {
    ...
    vector<Info> infos(0);
    infos.push_back({0, X[0]});
    infos.push_back({X[0], X[0]});

    ll ans = X[0] * 2LL;
    print(ans);
    rep(i, 1, N) {
        auto it = lower_bound(all(infos), Info{X[i], 0}, [&](const Info &a, const Info &b) {
            return a.x < b.x;
        });
