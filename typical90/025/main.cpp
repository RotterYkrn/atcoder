...
int main() {
    ...
    auto mul = mkvec<ll>(digit);
    ll tmp = B;
    rep(i, digit) {
        mul[i] = tmp % 10;
        tmp /= 10;
    }
    rrep(i, digit - 1) mul[i] *= mul[i + 1];

    ll cnt = 0;
    if (mul[0] == 0) cnt++;
    ll m = B + 1;
    mul[0] += (digit > 1 ? mul[1] : 1LL);
    while (m < max) {
        if (m % 10 == 0) {
            int update = 0;
            ll add_ones = 1;
            ...
            do {
                tmp /= 10;
                update++;
            } while (tmp % 10 == 0 && (add_ones = add_ones * 10 + 1));
            if (update == digit - 1) mul[update] = tmp % 10;
            else mul[update] += mul[update + 1];
            rrep(i, update) mul[i] = mul[i + 1];
            m += add_ones;
        } else {
            if (mul[0] == m - B) cnt++;
            m++;
