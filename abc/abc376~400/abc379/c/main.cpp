...
int main() {
    inputi(N,M);
    auto X = inputv<ll>(M), A = inputv<ll>(M);
    auto xa = mkvec<pll>(M);
    ...
    if (xa[0].first != 1) {
        ...
        if (xa[M - 1].first != N) xa.pb({N, 0});
        bool ena = true;
        rep(i, xa.size() - 1) {
            ll nex = xa[i + 1].first - xa[i].first;
            if (xa[i].second < nex) {
                ena = false;
                break;
            }
            ans += (nex * (nex - 1) / 2) + (xa[i].second - nex) * nex;
            xa[i + 1].second += xa[i].second - nex;
        }
        if (ena && xa[xa.size() - 1].second == 1) print(ans);
        else print(-1);
    }
