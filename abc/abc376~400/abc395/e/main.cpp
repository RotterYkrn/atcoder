...
int main() {
    ...
    auto compair = [](const State &a, const State &b) {
        return a.cost > b.cost;
    };

    auto costs = mkvec<ll>(N, -1);
    priority_queue<State, vector<State>, decltype(compair)> que{compair};
    que.push({0, 0, 0});
    while (!que.empty()) {
        auto [v, cost, dir] = que.top(); que.pop();
        ...
        for (auto nv : graph[v][dir]) {
            if (costs[nv] <= cost) continue;
            que.push({nv, cost + 1LL, dir});
        }
        for (auto nv : graph[v][1 - dir]) {
            if (costs[nv] <= cost) continue;
            que.push({nv, cost + X + 1LL, 1 - dir});
        }
    }
