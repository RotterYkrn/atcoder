...
int main() {
    ...
    auto ant = mkvec<ll>({2,0});
    rep(i, N) {
        if (S[i] == '1') ant[1].pb(X[i]);
        else ant[0].pb(X[i]);
    }
    sort(all(ant[0]));
    sort(all(ant[1]));
    ...
    bool it_end = false;
    rep(i, ant[1].size()) {
        auto it = lower_bound(all(ant[0]), ant[1][i]);
        if (it_end) {
            ans += (ll)distance(it, ant[0].end());
        } else {
            while (it != ant[0].end()) {
                if (*it - ant[1][i] <= T * 2) ans++;
                else break;
                ++it;
            }
            if (it == ant[0].end()) it_end = true;
        }
    }
