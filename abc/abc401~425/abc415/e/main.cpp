...
int main() {
    inputi(H, W);
    auto A = inputv<ll>({H, W});
    auto P = inputv<ll>(H + W - 1);

    auto compare = [](State a, State b) {
        return a.dx > b.dx;
    };

    auto seen = mkvec<pll>({H,W}, {infl, infl});

    priority_queue<State, vector<State>, decltype(compare)> q(compare);
    q.push({0, 0, 0, 0});
    while (!q.empty()) {
        auto [i, j, x, dx] = q.top(); q.pop();

        if (dx >= seen[H - 1][W - 1].second) {
            break;
        }
        ...
        if (ndx > seen[i][j].second) {
            continue;
        } else if (ndx == seen[i][j].second && nx <= seen[i][j].first) {
            continue;
        }
        seen[i][j] = {nx, ndx};

        if (i < H - 1) q.push({i + 1, j, nx, ndx});
        if (j < W - 1) q.push({i, j + 1, nx, ndx});
    }
    print(seen[H - 1][W - 1].second);
