...
int main() {
    ...
    const ll CENTER_ELEM = N * 500 + 10;
    const ll MAX_ELEM = CENTER_ELEM * 2 + 1;

    auto dp = mkvec<ll>({N + 1, MAX_ELEM}, -1);
    dp[0][CENTER_ELEM] = 0;
    rep(i, 1, N + 1) {
        rep(j, MAX_ELEM) {
            if (dp[i - 1][j] == -1) continue;
            if (j - W[i - 1] >= 0) {
                chmax(dp[i][j - W[i - 1]], dp[i - 1][j] + H[i - 1]);
            }
            if (j + W[i - 1] < MAX_ELEM) {
                chmax(dp[i][j + W[i - 1]], dp[i - 1][j] + B[i - 1]);
            }
        }
    }
    // print(dp);
