...
int main() {
    ...
    queue<Info> que;
    auto min_dist = mkvec<pii>(N, {inf, inf});
    rep(i, N) {
        if (S[i] == 'S') {
            min_dist[i] = {0, 0};
            que.push({i, 0});
    ...
    while (!que.empty()) {
        ...
        for (auto u : g[v]) {
            print(v, u, d);
            int tmp = min_dist[u].first;
            if (chmin(min_dist[u].first, d + 1)) {
                chmin(min_dist[u].second, tmp);
                que.push({u, d + 1});
            } else if (chmin(min_dist[u].second, d + 1)) {
                que.push({u, d + 1});
            }
        }
    }
    ...
    rep(i, N) {
        if (S[i] == 'D') {
            print(min_dist[i].first + min_dist[i].second);
        }
    }
