...
Info op(Info a, Info b) {
    return Info{a.r + b.r, a.c + b.c, max(a.d, b.d) + 1, a.exist || b.exist};
}
...
Info mapping(F f, Info x) {
    Info res = {x.r + f.r, x.c + f.c, x.d, x.exist};
    if (x.d == 0) {
        res.exist = (res.r == f.R && res.c == f.C);
    }
    return res;
}
...
F composition(F f1, F f2) {
    return F{f1.r + f2.r, f1.c + f2.c, R, C};
}
...
int main() {
    ...
    lazy_segtree<Info, op, e, F, mapping, composition, id> seg(N);
    auto ans = mkvec<int>(N);
    rep(i, N) {
        if (S[i] == 'N') {
            seg.apply(0, i, {-1, 0, R, C});
        } else if (S[i] == 'E') {
            seg.apply(0, i, {0, 1, R, C});
        } else if (S[i] == 'S') {
            seg.apply(0, i, {1, 0, R, C});
        } else if (S[i] == 'W') {
            seg.apply(0, i, {0, -1, R, C});
        }
        ans[i] = seg.all_prod().exist ? 1 : 0;
    }
