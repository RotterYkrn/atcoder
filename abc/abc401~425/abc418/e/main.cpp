...
int main() {
    inputi(N);
    auto pos = mkvec<pll>(N);
    rep(i, N) {
        inputll(x, y);
        pos[i] = {x, y};
    }

    map<pll, vector<pii>> mp;
    rep(i, N - 1) {
        rep(j, i + 1, N) {
            ll x = pos[i].first - pos[j].first;
            ll y = pos[i].second - pos[j].second;
            if ((x < 0 && y < 0)
            || (x < 0 && y > 0)) {
                x *= -1LL;
                y *= -1LL;
    ...
    struct MyHash {
        size_t operator()(const vector<int> &v) const {
            int hash_num = 0;
            rep(i, 4) {
                hash_num ^= hash<int>()(v[i]);
            }
            return hash_num;
        }
    };

    unordered_set<vector<int>, MyHash> st;
    for (auto [k, v] : mp) {
        rep(i, v.size() - 1) {
            rep(j, i + 1, v.size()) {
                vector<int> vec = {v[i].first, v[i].second, v[j].first, v[j].second};
                sort(all(vec));
                st.insert(vec);
            }
        }
