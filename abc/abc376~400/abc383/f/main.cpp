...
struct Item {
    ll P, U, C;
    Item(ll P, ll U, ll C) : P(P), U(U), C(C) { }
    Item(ll (&item)[3]) : P(item[0]), U(item[1]), C(item[2]) { }
};


int main() {
    inputll(N, X, K);
    auto items = mkvec<Item>(N);
    rep(i, N) {
        inputll(p, u, c);
        items[i] = {p, u, c};
    }

    auto dp = mkvec<pair<ll, set<ll>>>({N + 10, X + 10}, {-1, set<ll>()});
    dp[0][0] = {0, set<ll>()};
    for (int i = 1; i * items[0].P <= X; i++) {
        dp[0][i * items[0].P].first = items[0].U * i;
        dp[0][i * items[0].P].second.insert(items[0].C);
    }

    rep(i, 1, N) {
        auto [p, u, c] = items[i];
        rep(x, X + 1) {
            if (dp[i - 1][x].first == -1) continue;
            auto [bs, bt] = dp[i - 1][x];
            for (int j = 0; j * p <= X; j++) {
                auto nx = x + j * p;
                if (dp[i][nx].first == -1) {
                    dp[i][nx].first = bs + u * j;
                    dp[i][nx].second = bt;
                    dp[i][nx].second.insert(c);
                } else {
                    auto [cs, ct] = dp[i][nx];
                    auto t = bt;
                    t.insert(c);
                    if (bs + u * j + t.size() * K > cs) {
