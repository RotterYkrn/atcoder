...
int main() {
    ...
    ll ans = 0;
    auto f = [&](auto f, int d, int now_d, ll x) -> bool {
        if (x > N) {
            return false;
        }
        ...
        if (now_d <= (d - 1) / 2) {
            rep(n, (now_d != 0 ? 0 : 1), 10) {
                int d1 = now_d, d2 = d - now_d - 1;
                ll fx = x;
                if (d1 != d2) {
                    fx += n * pow10[d1] + n * pow10[d2];
                } else {
                    fx += n * pow10[d1];
                }
                if (!f(f, d, now_d + 1, fx)) {
            ...
            auto trans_x = trans(A, x);
            bool ok = true;
            rep(i, trans_x.size()) {
                if (trans_x[i] != trans_x[trans_x.size() - 1 - i]) {
                    ok = false;
                    break;
                }
            }
