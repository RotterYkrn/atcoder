...
int main() {
    ...
    rep(i,(int)imos.size()) rep(j,1,(int)imos[i].size()) imos[i][j] += imos[i][j - 1];
    rep(i,1,(int)imos.size()) rep(j,(int)imos[i].size()) imos[i][j] += imos[i - 1][j];

    auto ans = mkvec<int>(N + 1);
    rep(i,(int)imos.size()) rep(j,(int)imos[i].size()) ans[imos[i][j]]++;

    rep(i,1,(int)ans.size()) print(ans[i]);
