...
int main() {
    inputi(N, Q);
    dsu uf(N);
    vector<int> black_points(N, 0), is_black(N, 0);
    rep(Q) {
        inputi(q);
        if (q == 1) {
            inputi(u, v);
            ...
            int p = black_points[uf.leader(u)] + black_points[uf.leader(v)];
            black_points[uf.merge(u, v)] = p;
        } else if (q == 2) {
            inputi(v);
            v--;
            if (is_black[v]) {
                black_points[uf.leader(v)]--;
            } else {
                black_points[uf.leader(v)]++;
            }
            is_black[v] ^= 1;
            ...
            inputi(v);
            v--;
            YesNo(black_points[uf.leader(v)] > 0);
        }
    }
