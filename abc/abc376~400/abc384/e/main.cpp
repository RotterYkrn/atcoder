...
struct Slime {
    int y, x;
    ll s;
    Slime(int y, int x, ll s) : y(y), x(x), s(s) { }
};
...
int main() {
    ...
    inputll(X);
    inputi(P, Q); P--; Q--;
    auto S = inputv<ll>({H,W});

    auto compair = [](const Slime &a, const Slime &b) {
        return a.s > b.s;
        };

    ll str = S[P][Q];
    auto seen = mkvec<int>({H, W});
    seen[P][Q] = 1;
    priority_queue<Slime, vector<Slime>, decltype(compair)> que{compair};
    rep(d, 4) {
        int ni = P + dy[d], nj = Q + dx[d];
        if (ir(ni, H) && ir(nj, W)) {
            que.push(Slime(ni, nj, S[ni][nj]));
            seen[ni][nj] = 1;
        }
    }
    while (!que.empty() && que.top().s <= ((str - 1L) / X)) {
        ...
        str += slime.s;
        rep(d, 4) {
            int ni = slime.y + dy[d], nj = slime.x + dx[d];
            if (ir(ni, H) && ir(nj, W) && !seen[ni][nj]) {
                que.push(Slime(ni, nj, S[ni][nj]));
                seen[ni][nj] = 1;
            }
        }
