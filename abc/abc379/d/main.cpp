...
int main() {
    ...
    rep(Q) {
        ...
        if (q == 1) {
            ...
            if (pl.size() > 1 && ad != 0) {
                rep(i, ad - 1) pl[i + 1] += pl[i];
                rep(i, (size_t)ad, pl.size()) pl[i] += pl[ad - 1];
                ad = 0;
            } else {
                rep(i, pl.size()) pl[i] += T;
                T = 0;
            }
            inputll(H);
            
            int left = 0;
            int right = (int)pl.size() - 1;
            while (left < right) {
            ...
            if (pl[left] >= H) print(pl.size() - left);
            else print(0);
            pl.erase(pl.begin() + left, pl.end());
        }
    }
