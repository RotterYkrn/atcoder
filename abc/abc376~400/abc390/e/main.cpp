...
struct Vitamin {
    ...
    bool operator==(const Vitamin &rhs) const {
...
int main() {
    ...
    auto foods = mkvec<Food>(N);
    rep(i, N) {
        cin >> foods[i].v >> foods[i].a >> foods[i].c;
    }

    Vitamin init = {-1, -1, -1};
    auto dp = mkvec<Vitamin>({N, X + 1}, init);
    auto f = [&](auto self, int i, int x) -> Vitamin {
        if (i == 0) return {0, 0, 0};
        if (dp[i - 1][x] != init) return dp[i - 1][x];

        auto res = self(self, i - 1, x);

        if (foods[i - 1].c <= x) {
            auto tmp = self(self, i - 1, x - foods[i - 1].c);
            if (foods[i - 1].v == 1) {
                tmp.v1 += foods[i - 1].a;
                if (tmp.v1 > res.v1) {
                    res = tmp;
                }
            } else if (foods[i - 1].v == 2) {
                tmp.v2 += foods[i - 1].a;
                if (tmp.v2 > res.v2) {
                    res = tmp;
                }
