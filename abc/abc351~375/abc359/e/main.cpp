...
ll opH(ll a, ll b) { return max(a, b); }
ll e() { return 0L; }

ll opV(ll a, ll b) { return a + b; }
...
int main() {
    ...
    segtree<ll, opH, e> segH(N);
    rep(i, N) {
        segH.set(i, H[i]);
    }

    lazy_segtree<ll, opV, e, ll, mapping, composition, id> segV(N + 1);

    auto ans = mkvec<ll>(N + 1);
    rep(i, N) {
        auto hi = segH.max_right(0, [&](ll x) { return x >= H[i]; });
        auto v = segV.prod(hi, i + 1);
        ans[i + 1] = ans[i] + H[i] * (i - hi + 1) - v + 1;
        segV.apply(hi, i + 1, H[i]);
        segV.set(i + 1, 1L);
    }

    print(vll(rtoit(ans, 1)));
