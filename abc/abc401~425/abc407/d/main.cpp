...
int main() {
    ...
    auto blocked = mkvec<int>({H,W});

    auto rec = [&](auto self, int i, int j, ull x) -> ull {
        if (j == W) {
            j = 0;
            i++;
        }
        ...
        chmax(res, self(self, i, j + 1, x));
        if (!blocked[i][j] && j + 1 < W && !blocked[i][j + 1]) {
            blocked[i][j] = blocked[i][j + 1] = 1;
            chmax(res, self(self, i, j + 2, x ^ A[i][j] ^ A[i][j + 1]));
            blocked[i][j] = blocked[i][j + 1] = 0;
        }
        if (!blocked[i][j] && i + 1 < H && !blocked[i + 1][j]) {
            blocked[i][j] = blocked[i + 1][j] = 1;
            chmax(res, self(self, i, j + 1, x ^ A[i][j] ^ A[i + 1][j]));
            blocked[i][j] = blocked[i + 1][j] = 0;
        }

        return res;
    };

    print(rec(rec, 0, 0, xors));
